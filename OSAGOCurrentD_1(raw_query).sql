...
FROM(
	...
	FROM (
		...
		FROM Products AS P
			...
			INNER JOIN(
				SELECT Pg.gid AS ProgramGID, Pg.ProductGID,
					...
					MAX(CASE PTPV.ParameterGID
						WHEN 'D3CC1D06-C28E-47D3-8180-61921FFCBE84' THEN TRY_CONVERT(int, PPV.Value)
					END) AS b_m,
					MAX(CASE PTPV.ParameterGID
					...
					END) AS need_to,
					MAX(CASE PTPV.ParameterGID
						WHEN 'D42F6AA7-09B6-4B64-A43D-3C241B41C202' THEN TRY_CONVERT(date, PTPV.ParameterValue)
						ELSE CAST(NULL AS date)
					END) AS date_next_to,
					MAX(CASE PTPV.ParameterGID
						WHEN 'E681EBF3-B5A1-436E-852F-AB1C288B8A0B' THEN TRY_CONVERT(int, PTPV.ParameterValue, 0)
				...
				FROM Programs AS Pg
					...
					INNER JOIN Coefficients AS C ON C.ProgramGID=Pg.gid
						AND C.Deleted=0
					INNER JOIN meta.ProgramCoefficients AS PC ON PC.gid=C.ProgramCoefficientGID
					INNER JOIN ProgramToParameterValues AS PTPV ON PTPV.ProgramGID=Pg.gid
						AND PTPV.Deleted=0
					LEFT JOIN meta.ProgramParameterValues AS PPV ON PPV.gid=PTPV.ParameterValueGUIDKey
						AND PPV.ProgramParameterGID=PTPV.ParameterGID
				WHERE Pg.Deleted=0
				GROUP BY Pg.gid, Pg.ProductGID
			) AS ProgramData ON ProgramData.ProductGID=P.gid
		--Автомобиль
			INNER JOIN InsuranceObjects AS IOb ON IOb.ProgramGID=ProgramData.ProgramGID
				AND IOb.Deleted=0
			...
			LEFT JOIN( Metis.dbo.Vehicles AS V
				LEFT JOIN DWH.Metis.Settlements AS S ON S.gid=V.SettlementGID
		...
		--Разбираем платежи
			INNER JOIN(
				SELECT PPay.ProgramGID, SUM(PPay.Value) AS PValue, SUM(ISNULL(PTR.Value, 0)) AS RValue
						...
						WHERE PTR.Deleted=0
							AND PTR.RealPaymentGID IN(
								...
								FROM Ganimed.dbo.BankPayments AS BP
									INNER JOIN Ganimed.dbo.Moneys AS M ON M.FromGID=BP.gid
										AND M.Deleted=0
								WHERE BP.Deleted=0
							)
			...
			) AS PPay ON PPay.ProgramGID=ProgramData.ProgramGID
		--Покрытие
			INNER JOIN(
				...
				FROM Covers AS C
					...
					LEFT JOIN meta.FranchiseTypeValues AS FTV ON FTV.FranchiseTypeGID=F.FranchiseTypeGID
			...
			LEFT JOIN Metis.dbo.Persons AS Pers ON Pers.gid=F.gid
			LEFT JOIN Metis.dbo.JuridicalPersons AS JPer ON JPer.gid=F.gid
			LEFT JOIN(
				SELECT D.ObjectGID,
		...
		--Current Status
			LEFT JOIN(
				SELECT P.gid, BP.PolisNumber, COUNT(ESAF.ProductGID) AS Value
				FROM Products AS P
					LEFT JOIN EditsSupplementaryAgreementFields AS ESAF ON ESAF.ProductGID=P.gid
						AND ESAF.SupplementaryAgreementFieldGID NOT IN(
							'F08678F7-039A-42FA-808C-2D13B3D908B1',
							'34D0D2B9-2204-4527-9519-31C0EEEF0FB2'
						)
					INNER JOIN Products AS BP ON BP.gid=P.BaseProductGID
						AND BP.Deleted=0
				WHERE P.Deleted=0
					AND P.BaseProductGID IS NOT NULL
				GROUP BY P.gid, BP.PolisNumber
			) AS BP ON BP.gid=P.gid
		WHERE P.Deleted=0
			AND ISNULL(P.LastModifiedDate, P.CreateDate) BETWEEN @StartDate 
			AND DATEADD(s,-1,DATEADD(d,1, @EndDate))
	) AS PolisData
) AS PolisData